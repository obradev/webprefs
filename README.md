...
3. Click on the '+' icon in the top-left corner, then click 'Install package from git URL'.
4. Paste 'https://github.com/obradev/WebPrefs.git' into the textbox.
5. It should be installed now, you can head over to 'Samples' for an optional demo scene.
...
1. `WebPrefs.Load<T>(string key))`, if we wanted to for example load those coins from before, we would do `int coins = WebPrefs.Load<int>("coins")` (we use `<int>` to specify we want the data back as an int value)
...
`HasKey(string key)` - Checks if a value with the key exists on the device. (returns a simple bool value)


**DELETING VALUES:**

`DeleteKey(string key)` - Clears the data stored with that key.

`ClearAllData()` - Clears all the data.

**OTHER METHODS:**

`Reload()` - Reloads the data from the backup IndexedDB, or saves the PlayerPrefs data if not on a web build.

`GetRawData()` - Returns the full serialized string from the `localStorage`, practically useless, its very optimized for file size and harder to read on its own.
